...
async def _play_and_record(*, game, seed, evaluator, n_sims, rec, sem, active,
                           self_play=False, game_deadline_seconds=None):
    async with sem:
        active["n"] += 1
        ...
        try:
            result = await play_one_async_game(
                ...
                deadline_seconds=game_deadline_seconds)
...
async def _run_async(*, out, checkpoint, num_games, n_sims, n_concurrent,
    ...
    tasks = [_play_and_record(game=game, seed=s, evaluator=evaluator,
                              ...
                              self_play=self_play,
                              game_deadline_seconds=game_deadline_seconds)
             for s in seeds]
    try:
...
def run_self_play(*, out_root: Path, checkpoint: Path, num_games: int = 64,
                  ...
                  per_game_mb: float = 50.0, self_play: bool = False,
                  game_deadline_seconds: float | None = None) -> Path:
...
def cli_main():
    ...
    p.add_argument("--game-deadline-seconds", type=float, default=None,
                   help="per-GAME wall-clock cap: a game exceeding it stops and "
                        "is recorded timed_out (avoids one straggler stalling the "
                        "whole stage). None = no per-game cap.")
    ...
    out = run_self_play(
        ...
        game_deadline_seconds=args.game_deadline_seconds)
